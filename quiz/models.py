...
class Quiz(models.Model):
    ...
    random_order = models.BooleanField(
        ...
        verbose_name=_("Рандомная сортировка"),
        help_text=_("Рандомная сортировка вопросов"))

    max_questions = models.PositiveIntegerField(
        blank=True, null=True, verbose_name=_("Максимально вопросов"),
        help_text=_("Количество вопросов, на которые нужно ответить при каждой попытке."))

    answers_at_end = models.BooleanField(
        blank=False, default=False,
        help_text=_("Отобразить ответ вконце"),
        verbose_name=_("Ответы вконце"))

    exam_paper = models.BooleanField(
        blank=False, default=False,
        help_text=_("Если да, то будет записываться"),
        verbose_name=_("Количество попыток"))

    single_attempt = models.BooleanField(
        blank=False, default=False,
        help_text=_("Разрешить только одну попытку?"),
        verbose_name=_("Одна попытка"))

    pass_mark = models.SmallIntegerField(
        blank=True, default=0,
        verbose_name=_("Балл"),
        help_text=_("Процент необходимый для прохождения теста"),
        validators=[MaxValueValidator(100)])

    success_text = models.TextField(
        blank=True, help_text=_("Текст о прохождение теста"),
        verbose_name=_("Тест пройден"))

    fail_text = models.TextField(
        verbose_name=_("Тест не пройден"),
        blank=True, help_text=_("Текст о том что тест не пройден"))

    draft = models.BooleanField(
        blank=True, default=False,
        verbose_name=_("Черновик"),
        help_text=_("Отображать тест для пользователя? По дефолту - отображать"))
    ...
    def save(self, force_insert=False, force_update=False, *args, **kwargs):
        ...
        if self.pass_mark > 100:
            raise ValidationError('%s больше 100' % self.pass_mark)
...
class Question(models.Model):
    ...
    quiz = models.ManyToManyField(Quiz,
                                  verbose_name=_("Тестовые задания"),
                                  blank=True)
    ...
    explanation = models.TextField(max_length=2000,
                                   blank=True,
                                   help_text=_("Объяснение вопроса, котоорое будет показано после ответа на вопрос"),
